<template>
<!--XCvalue{{ cvalue }} V:{{v}} Modelvalue:{{ modelValue }} sv {{ column }} -->
<!--        @update:modelValue="handleInput" -->
<!-- V-{{v}} S-{{ search }}  Modelvalue:{{ modelValue }} BV:{{ buttonvalue }} Options: {{ options }} VM: {{ v-model }} -->
<!--
        :hint = this.labelmessage
    this.$refs.input.reset()
    @keydown.enter ="modelValue = '', search = '', options = []"
    clearable
    @click:clear = "modelValue = '', search = '', options = []"

-->
    <!--
        no-data-text = "nodatat"
    @keydown.enter ="search = '', modelValue = '', v = [], options = []"
    clearable
    @click:clear = "search = '', modelValue = '', v = [], options = []"

    -->
<!-- <v-btn @click="clearInput">Reset</v-btn> -->
<!-- ref="input"
        prepend-inner-icon="mdi-magnify"
    @click:prepend-inner="handleInput(value)"
@update:modelValue="handleInput" 
    clearable="search"   

        prepend-inner-icon="mdi-eraser"
    @click:prepend-inner="search = '', modelValue = '', v = [], options = []"
-->
    <!-- <v-btn
        class="mb-1 mr-1"
    ...
    >
        ...
        clearable="search"
            @changed="search = ''"
        v-model = "searchvalue"
<div v-for="row in options" :key="row.value">{{row.title}}</div>
    append-inner-icon="mdi-magnify"
    @click:append-inner="handleInput(modelValue)"
-->
<v-autocomplete
hide-no-data
variant="outlined"
