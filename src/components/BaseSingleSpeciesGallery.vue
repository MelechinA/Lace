...
variant="tonal"
      color="regular"
@click="$router.push(tableurlid+'/'+img.data.id )" target="_blank"
>
LID{{img.data.lid}}
</v-btn>
  </v-img>
...
>
  ...
  height="360"
    ...
    :src= prefixsmall+img.link
    :title=img.data.species.name
    @click="overlay = !overlay, imgsrc = prefixmiddle+img.link"
    >
    <template v-slot:placeholder>
      <div class="d-flex align-center justify-center fill-height">
        <v-progress-circular
...
*/
  ...
  //        let querydata = {sort: [{fieldname: "species", dir: true}]};
          ...
          //  console.log(key, this.filters[key]);
            ...
            querydata['f_pictures'] = "isnotnull";
...
//          this.imglinksmassif = [];
          ...
          if (row.pictures != undefined) {  // checking undefined values - so ok? 
        let images=JSON.parse(row.pictures);
