...
<v-card-text>
    ...
    >
<v-tab :value="fishhome" @click="$router.push('/fish')">
<span class="text-regular">Fish </span> home
</v-tab>

<!-- <v-tab 
  value="fish_species_main"
  @click="getAllMeta"
>
Species
<v-menu activator="parent">
  <v-list>
    <v-list-item
      ...
      :key="index"
      :value="index"
      @click="$router.push('/fish/table/'+item.idlink)"
    >
...
<!-- <v-tab value="fish_specimen_others">
      Others
      <v-menu activator="parent">
        <v-list>
          <v-list-item
...
<v-btn
    ...
    >
        ...
        >
          <v-list >
            <v-tab class="px-2" @click.stop="drawer = !drawer" value="lhome" @click="$router.push('/fish/')"><span class="text-regular">Fish </span><span class="px-0"> home</span></v-tab>
            <!-- <v-tab 
              value="fish_species_main"
              @click="getAllMeta"
            >
            ...
            <v-menu activator="parent" location=center>
              <v-list>
                <v-list-item
                  ...
                  :key="index"
                  :value="index"
                  @click="$router.push('/fish/table/'+item.idlink), drawer = !drawer"
                >
                  <v-list-item-title>{{ item.title }}</v-list-item-title>
                </v-list-item>
              </v-list>
...
/*               Т.к. именно здесь формируются линки, то я должен здесь получать список возможных линков и их имен (чтобы заменить искуственный массив items) (ну потом можно это куда-то пренести, но суть надо отрабоать здесь).
               Чтобы получить массив со списком этих вот - aaData.id, aaData.title, aaData.name - я должен получить structure  без привязки к ID 
               - те. получить вообще всю "структуру структур"
